...
function wfu_view_log($page = 1, $only_table_rows = false, $located_rec = -1) {
	...
	foreach ( $filerecs as $ind => $filerec ) {
		...
		elseif ( $filerec->action == 'filetransfer' ) {
			...
			if ( $filedata && isset($filedata["log"]) ) {
				...
				if ( $service == "" ) $remarks = "\n\t\t\t\t\t\t".'<label>No data!</label>';
				elseif ( $transferred ) $remarks = "\n\t\t\t\t\t\t".'<label>Το '.$service.': '.$destination.( $new_filename == "" ? "" : '/'.$new_filename ).'</label>';
				else {
					$logerror = "";
					switch($error) {
					...
					$remarks = "\n\t\t\t\t\t\t".'<label>Transfer tο '.$service.' aborted: '.$logerror.'</label>';
				}
			}
		}
		...
		elseif ( in_array($filerec->linkedto, $deletedfiles) || in_array($filerec->idlog, $deletedfiles) ) $echo_str .= "\n\t\t\t\t\t\t".'<span>'.$displayed_path.'</span>';
			...
			if ( $filerecs[$newestind]->date_to == "0000-00-00 00:00:00" && $filerec->filepath == $filerecs[$newestind]->filepath && wfu_file_exists($file_abspath, "wfu_view_log") ) {
				if ( !isset($filecodes[$lid]) ) {
					$filecodes[$lid] = ( wfu_check_file_remote($file_abspath) !== false ? "remote:".$filerec->idlog : wfu_safe_store_filepath($filerec->filepath) );
				}
